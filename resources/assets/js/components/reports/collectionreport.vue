           ...
           <Col  class="dream-input-main " span="22" offset="1">
                ...
                <Row>
                    <Col span="10" offset="15" >
                        <h3>Total Collection Amount: {{totalCollectionPrice}} </h3>
                    </Col>
                </Row>
    ...
    export default {
        ...
        computed: {
            ...
            totalCollectionPrice(){
                ...
                for(let i = 0;i<this.searchData.length;i++){
                    ...
                    totalCollection+= parseInt(this.searchData[i].paidAmount,10);
                }
        ...
        methods: {
            ...
            async changedcustomer(k)
            {
                ...
                let {data} =await  axios({
                    method: 'get',
                    url:`/app/payment/getOutstanding/${this.formValue.customer_id}`
                })
                this.setcustomer(this.formValue.customer_id)
                this.currentcustomer.outStanding=data.outStanding
            ...
            async makePurchase(){
                ...
                if( !this.totalQuantity || !this.totalPrice || !this.formValue.customer_id|| !this.formValue.date)
                {
                    this.loading=false
                ...
                else
                {
                    ...
                    try{
                        ...
                        data.data.customerName=data.data.customer.customerName
            ...
            async remove(){
                ...
                try{
                    let {data} =await  axios({
                        method: 'get',
                        url:`/app/collectionCancel/${this.UpdateValue.id}`,
                    })
        ...
        async created()
        {
            this.$store.dispatch('updateHeader','Collection Report');
            this.ls();
            const end = new Date();
			const start = new Date();
			start.setTime(start.getTime() - 3600 * 1000 * 24 * 30);
            ...
            try{
                let {data} =await  axios({
                    method: 'get',
                    url:`/app/collectionList/${date2}/${date2}`
